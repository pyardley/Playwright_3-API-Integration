...
test.describe('Playwright APIRequestContext for CRUD', () => {
  test('Submit the contact form and verify the underlying /messages API call and response shape', async ({
    ...
    await expect(contactPage.getHeading()).toBeVisible();
    ...
    await contactPage.fillForm({ firstName, lastName, email, subject: 'Customer service', message });

    await expect(contactPage.getFirstNameField()).toHaveValue(firstName);
    await expect(contactPage.getLastNameField()).toHaveValue(lastName);
    await expect(contactPage.getEmailField()).toHaveValue(email);
    ...
    await expect(contactPage.getSubjectSelect()).toHaveValue('customer-service');
    await expect(contactPage.getMessageField()).toHaveValue(message);
    ...
    await expect(contactPage.getConfirmationAlert()).toHaveText(
